...
//---LED SETUP STUFF
#define LED_COUNT 36          //FOR TESTING w/ SIGN
...
// make the code dynamically support different numbers of segments
// try pulsing the colors (within the core base color) using a sin function
// add duration support
// add parameters for segments, duration, and speed (delay)
// also add a parameter for the desired circle LED array?

  idex++;
  int ghue = (thishue + 96) % 255;
  int bhue = (thishue + 160) % 255;
  int N3  = int(LED_COUNT/3);
  int N6  = int(LED_COUNT/6);  
  int N12 = int(LED_COUNT/12);  
  uint8_t segments = 4;
  uint8_t offset = (int)(LED_COUNT / segments);
  uint8_t start = idex % LED_COUNT;
  
  for(int i = 0; i < offset; i++ ) {
//    int j0 = (idex + i + LED_COUNT - N12) % LED_COUNT;
    int j0 = idex % LED_COUNT;
    int j1 = (j0 + offset) % LED_COUNT;
    int j2 = (j1 + offset) % LED_COUNT;    
    int j3 = (j2 + offset) % LED_COUNT;    
    leds[j0] = CHSV(thishue, thissat, 255);
    leds[j1] = CHSV(ghue, thissat, 255);
    leds[j2] = CHSV(230, thissat, 255);    
    leds[j3] = CHSV(bhue, thissat, 255);    
    // leds[(start + (offset * i)) % LED_COUNT] = hColors[i];
...
//  LEDS.addLeds<WS2801, LED_CK, LED_DT, RBG, DATA_RATE_MHZ(1)>(leds, LED_COUNT);
	LEDS.addLeds<LPD8806, LED_DT, LED_CK, BRG>(leds, LED_COUNT);
